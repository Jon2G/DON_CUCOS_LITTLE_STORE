...
-----CUSTOMER-----
GO
CREATE PROCEDURE SP_ABC_CUSTOMER (@ID INT, @NAME VARCHAR(100),@NOTES VARCHAR(MAX),@PICTURE VARCHAR(MAX))
AS
BEGIN
	IF(@ID<=0)
	BEGIN
		INSERT INTO CUSTOMERS(NAME,NOTES,PICTURE) VALUES(@NAME,@NOTES,@PICTURE) 
	END ELSE
	BEGIN
		UPDATE CUSTOMERS SET NAME= @NAME,NOTES=@NOTES,PICTURE=@PICTURE WHERE ID=@ID
	END
END;
GO
...
BEGIN
	...
	END ELSE
	BEGIN
		UPDATE SUPPLIERS SET NAME=@NAME,CELLPHONE=@CELLPHONE,NOTES=@NOTES,PICTURE=@PICTURE WHERE ID=@ID;
	END
END;
GO
